...
function bindEvents() {
    ...
    qsa("input[name='shippingMethod']").forEach(r => {
        r.addEventListener("change", () => {
            ...
            CheckoutState.originalShipping =
                Number(
                    r.dataset.price || 0
                );
...
CheckoutState.shippingDiscount = 0;
...
function saveDraft() {
    ...
    d.originalShipping = CheckoutState.originalShipping;
    d.freeShipping = CheckoutState.freeShipping;
    d.shippingDiscount = CheckoutState.shippingDiscount;
...
function renderSummary() {
    ...
    const total = Math.max(
        0,
        subtotal -
        Number(CheckoutState.discount || 0) +
        finalShipping
    );
...
async function applyCoupon() {
    ...
    try {
        ...
        CheckoutState.shippingDiscount =
            Number(
