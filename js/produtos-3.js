...
/* ==========================================================
   ...
   Tipo de produto -> Categoria -> Subcategoria

   A tabela categories usa parent_id:
   nível 1 = Tipo
   nível 2 = Categoria
   nível 3 = Subcategoria
========================================================== */

function a3EscapeHtml(value) {
  return String(value ?? "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
...
function a3CategoryById(id) {
  return Admin3State.categories.find((item) => item.id === id) || null;
}

function a3CategoryChildren(parentId) {
  return Admin3State.categories
    .filter((item) => item.parent_id === parentId)
    .sort((a, b) => String(a.name).localeCompare(String(b.name), "pt-BR"));
}

function a3RootTypes() {
  return a3CategoryChildren(null);
}

function a3GetHierarchy(id) {
  const result = {
    type: null,
    category: null,
    subcategory: null
  ...
  while (current && !visited.has(current.id)) {
    ...
    const parent = a3CategoryById(current.parent_id);
...
function injectCatalogHierarchyStyles() {
  if (document.querySelector("#admin3CatalogHierarchyStyles")) return;

  const style = document.createElement("style");
  style.id = "admin3CatalogHierarchyStyles";
  style.textContent = `
    #admin3CatalogHierarchy {
      display: contents;
    }
    #admin3CatalogHierarchy > label {
      display: block;
    }
    #admin3CatalogHierarchy select:disabled {
      opacity: .55;
      cursor: not-allowed;
    }
    #admin3CatalogHierarchy small {
      display: block;
      margin-top: 6px;
      opacity: .65;
      font-size: .78rem;
...
function a3EnsureCatalogFields() {
  injectCatalogHierarchyStyles();
  const category = a3("#admin3Category");
  if (!category) return;

  if (a3("#admin3CatalogHierarchy")) return;

  const physicalType = a3("#admin3ProductType")?.closest("label");

  const wrapper = document.createElement("div");
  wrapper.id = "admin3CatalogHierarchy";
  wrapper.className = "admin3-catalog-hierarchy";
  wrapper.innerHTML = `
    <label>
      Tipo de produto
      <select id="admin3ProductKind" required>
        <option value="">Selecione o tipo</option>
      </select>
      <small>Ex.: Canecas, Xícaras, Garrafas.</small>
    </label>

    <label>
      Categoria
      <select id="admin3CatalogCategory" required disabled>
    ...
    <label>
      Subcategoria
      <select id="admin3Subcategory" required disabled>
        <option value="">Selecione a categoria primeiro</option>
      </select>
    </label>
  `;

  category.closest("label")?.replaceWith(wrapper);

  if (physicalType) {
    const textNode = Array.from(physicalType.childNodes).find(
      (node) => node.nodeType === Node.TEXT_NODE && node.textContent.trim()
    );
    if (textNode && textNode.textContent.includes("Tipo de produto")) {
      textNode.textContent = "Tipo de item";
    }
  }

  bindCatalogHierarchyEvents();
}

function bindCatalogHierarchyEvents() {
  const type = a3("#admin3ProductKind");
  const category = a3("#admin3CatalogCategory");
  const subcategory = a3("#admin3Subcategory");

  if (!type || !category || !subcategory) return;

  type.onchange = () => {
    renderCatalogCategorySelect(type.value);
    subcategory.innerHTML = `<option value="">Selecione a categoria primeiro</option>`;
    subcategory.disabled = true;
  };

  category.onchange = () => {
    renderCatalogSubcategorySelect(category.value);
  };
}

function renderCatalogHierarchy(selectedId = null) {
  a3EnsureCatalogFields();
  ...
  const hierarchy = selectedId ? a3GetHierarchy(selectedId) : {
    type: null,
    category: null,
    subcategory: null
  };
  ...
  type.innerHTML = `
    <option value="">Selecione o tipo</option>
    ${a3RootTypes().map((item) => `
      <option value="${item.id}">
        ${a3EscapeHtml(item.name)}
      </option>
    `).join("")}
  `;

  type.value = hierarchy.type?.id || "";

  renderCatalogCategorySelect(
    hierarchy.type?.id || "",
    hierarchy.category?.id || null
  );

  renderCatalogSubcategorySelect(
    hierarchy.category?.id || "",
    hierarchy.subcategory?.id || null
  );

  syncLegacyCategoryField();
}

function renderCatalogCategorySelect(typeId, selectedId = null) {
  const select = a3("#admin3CatalogCategory");
  const subcategory = a3("#admin3Subcategory");

  if (!select) return;

  const rows = typeId ? a3CategoryChildren(typeId) : [];
  ...
  if (subcategory && !selectedId) {
    subcategory.disabled = true;
    subcategory.innerHTML = `<option value="">Selecione a categoria primeiro</option>`;
  }
}

function renderCatalogSubcategorySelect(categoryId, selectedId = null) {
  const select = a3("#admin3Subcategory");

  if (!select) return;

  const rows = categoryId ? a3CategoryChildren(categoryId) : [];
...
function syncLegacyCategoryField() {
  const legacy = a3("#admin3Category");
  const subcategory = a3("#admin3Subcategory");
  const category = a3("#admin3CatalogCategory");
  const type = a3("#admin3ProductKind");
  ...
  /*
   * Mantemos admin3Category por compatibilidade com o código existente.
   * O valor salvo será sempre a subcategoria quando houver uma.
   */
  legacy.value =
    subcategory?.value ||
    category?.value ||
...
function validateCatalogHierarchy() {
  const type = a3("#admin3ProductKind");
  const category = a3("#admin3CatalogCategory");
  const subcategory = a3("#admin3Subcategory");
...
function a3HierarchyLabel(categoryId) {
  const hierarchy = a3GetHierarchy(categoryId);

  return [
    hierarchy.type?.name,
    hierarchy.category?.name,
    hierarchy.subcategory?.name
  ].filter(Boolean).join(" → ") || a3CategoryById(categoryId)?.name || "Sem categoria";
}
...
function renderCategoryOptions() {
  ...
  const all = Admin3State.categories;
  const roots = a3RootTypes();
      ...
      all
        ...
        .map((category) => `
          <option value="${category.id}">
            ${a3EscapeHtml(a3HierarchyLabel(category.id))}
          </option>
        `)
        .join("");
  }
  ...
  if (field) {
    field.innerHTML = all.length
      ? all.map((category) => `
          <option value="${category.id}">
            ${a3EscapeHtml(a3HierarchyLabel(category.id))}
          </option>
        `).join("")
      : `<option value="">Cadastre a hierarquia primeiro</option>`;
    field.style.display = "none";
    field.removeAttribute("required");
  }

  const currentId = Admin3State.currentProductId
    ? Admin3State.products.find((item) => item.id === Admin3State.currentProductId)?.category_id
    : null;

  renderCatalogHierarchy(currentId || null);
}
  ...
  return `
    <article class="admin3-product-card">
      ...
      <div class="admin3-badges">
        <span class="admin3-badge">${product.category_id ? a3HierarchyLabel(product.category_id) : "Sem categoria"}</span>
...
window.editAdmin3Product = async function(id) {
  ...
  renderCatalogHierarchy(product.category_id || null);
